...
describe('locating a field in the editor buffer', () => {
  ...
  it('spans a multi-line array value', () => {
    expect(slice('filament_cost')).toBe('"filament_cost": [\n        "249"\n    ]');
  });

  it('spans a nested object value rather than its inner key', () => {
    expect(slice('nested')).toBe('"nested": {\n        "layer_height": "9"\n    }');
  });
...
describe('profile leaf validation', () => {
  it('blocks invalid JSON and non-object JSON', () => {
    expect(validateProfileJson('{', { kind: 'process', original }).canSave).toBe(false);
    expect(validateProfileJson('[]', { kind: 'process', original }).canSave).toBe(false);
  });
  ...
  it('accepts a leaf that inherits type, and rejects a conflicting one', () => {
    ...
    expect(
      validateProfileJson(JSON.stringify({ ...inheritsType, type: 'filament' }), {
        kind: 'filament',
        original: inheritsType,
      }).canSave,
    ).toBe(true);
    expect(
      validateProfileJson(JSON.stringify({ ...inheritsType, type: 'process' }), {
        kind: 'filament',
        original: inheritsType,
      }).canSave,
    ).toBe(false);
  ...
  it('validates Orca bounds, enums, booleans, and value shapes', () => {
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          key: 'seam_slope_type',
          message: expect.stringContaining('Unrecognized value "not-an-orca-mode"'),
        }),
      ]),
    );
  });
  ...
  it('allows saving with warnings', () => {
    const result = validateProfileJson(JSON.stringify({ ...original, layer_height: '9' }), {
      kind: 'process',
      original,
      inherited: { layer_height: '0.2' },
    });
    expect(result.canSave).toBe(true);
    expect(result.findings.every(({ severity }) => severity === 'warning')).toBe(true);
  });
