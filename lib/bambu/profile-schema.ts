...
const LAYER_COUNT_KEY_RE = /(layers?$|_layers_|layer$|_octaves$|wall_loops|skirt_loops|wall_count)/;
const ENUM_KEY_RE =
...
function additionalSectionId(kind: ProfileKind, key: string): string {
  ...
  if (kind === 'filament') {
    ...
    if (/dry|safe|compatib|printable|soluble|ingredient|stickiness|HRC/.test(key)) {
  ...
  if (/infill|shell|surface|wall|bridge/.test(key)) return 'strength-additional';
...
export function collectChainKeys(chain: readonly InheritanceChainLevel[]): string[] {
...
export function buildCompleteUiTree(
  ...
  const rootKeys =
    slicer === 'bambu' ? (kind === 'filament' ? FILAMENT_ROOT_KEYS : PROCESS_ROOT_KEYS) : [];
      ...
      : curatedTree
          .map((group) => ({
            ...
            subgroups: group.subgroups
              .map((subgroup) => ({
                ...subgroup,
                properties: subgroup.properties.filter((property) => allKeys.has(property.key)),
              }))
  ...
  const mapped = new Set(
    visibleCuratedTree.flatMap((group) =>
      group.subgroups.flatMap((subgroup) => subgroup.properties.map((property) => property.key)),
    ),
  );
