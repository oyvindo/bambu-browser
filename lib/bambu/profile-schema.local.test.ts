...
const studioRoot =
  ...
  (process.platform === 'darwin'
    ...
    : process.platform === 'win32'
      ? path.join(process.env.APPDATA || path.join(homedir(), 'AppData', 'Roaming'), 'BambuStudio')
...
describe('installed Bambu Studio root schemas', () => {
  it.each([
    ['process', 'system/BBL/process/fdm_process_common.json', PROCESS_ROOT_KEYS],
    ['filament', 'system/BBL/filament/fdm_filament_common.json', FILAMENT_ROOT_KEYS],
  ] as const)(
    '%s manifest matches the installed common root',
    (_kind, relativePath, expectedKeys) => {
      const installedKeys = readRootKeys(relativePath);
      if (!installedKeys) {
        console.warn(`Skipping ${relativePath}: no Bambu Studio root at ${studioRoot}`);
        return;
      }
  ...
  it.each([
    ...
    (_kind, relativePath, expectedKeys) => {
      ...
      const html = renderToStaticMarkup(React.createElement(LocaleProvider, null, table));
