...
import { flattenSidebarRows, type SidebarSection, type SidebarRow } from './sidebarRows';
...
describe('sidebar keyboard rows', () => {
  it('places the next heading directly after the last profile of a group', () => {
    ...
    expect(rows[rows.indexOf('user/1/filament/last.json') + 1]).toBe('[process]');
  });
  ...
  it("puts a group's first profile right after its heading once expanded", () => {
    ...
    expect(rows[rows.indexOf('[filament_custom]') + 1]).toBe('user/1/filament/custom.json');
    expect(rows[rows.indexOf('user/1/filament/custom.json') + 1]).toBe('[filament_standard]');
  });
});
