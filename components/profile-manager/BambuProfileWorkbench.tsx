...
import { ChevronDown, HelpCircle, Loader2, RefreshCw, Server } from 'lucide-react';
...
import { flattenSidebarRows, type SidebarRow, type SidebarSection } from './sidebarRows';
...
export function BambuProfileWorkbench() {
  ...
  const sectionTriggerRefs = useRef(new Map<SidebarSection, HTMLButtonElement>());
  const [systemFilamentEntries, setSystemFilamentEntries] = useState<SystemFilamentEntry[]>([]);
  ...
  const compareFilamentPath =
    compareFilament.profilePath === selectedPath ? compareFilament.relativePath : null;
  ...
  type ConnectionCheckResult = { ok: true; root: string } | { ok: false; error: string };
  ...
  const loadApiConnection = useCallback(
    async (source: SlicerSource = slicer): Promise<ConnectionCheckResult> => {
      ...
      try {
        ...
        if (!health.ok) {
          const error = health.error || t('errors.serverCannotReadRoot', { root: health.root });
          setApiOk(false);
          setAccountNames([]);
        ...
        const error = e instanceof Error ? e.message : t('errors.cannotReachApi');
        setApiOk(false);
        setAccountNames([]);
        setStudioRootLabel('');
        setLayout(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      const storedSlicer =
        typeof window !== 'undefined' ? localStorage.getItem(SLICER_STORAGE_KEY) : null;
      const initialSlicer = isSlicerSource(storedSlicer) ? storedSlicer : DEFAULT_SLICER_SOURCE;
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        const { profiles: list } = await fetchApiProfilesForAccount(selectedUsername, slicer);
        if (!cancelled) {
          setProfiles(list);
          setSelectedPath(null);
          setChain([]);
        ...
        if (!cancelled) {
          setError(e instanceof Error ? e.message : t('errors.loadProfilesFailed'));
          setProfiles([]);
        }
      } finally {
  ...
  useEffect(() => {
    let cancelled = false;
    const compareArg = isCustomFilamentProfile && compareFilamentPath ? compareFilamentPath : null;
    const run = async () => {
      ...
      setError(null);
      try {
        const { chain: c } = await fetchApiResolve(selectedPath, slicer, compareArg);
        if (!cancelled) setChain(c);
      } catch (e) {
        if (!cancelled) {
          setChain([]);
          setError(e instanceof Error ? e.message : t('errors.resolveInheritanceFailed'));
        }
      } finally {
  ...
  }, [selectedPath, apiOk, t, isCustomFilamentProfile, compareFilamentPath, slicer]);
  ...
  const grouped = useMemo(() => {
    ...
    return Array.from(m.entries()).sort(([a], [b]) => SECTION_ORDER[a] - SECTION_ORDER[b]);
  }, [profiles]);
  ...
  const handleProfileKeyDown = useCallback(
    (event: KeyboardEvent<HTMLButtonElement>, section: SidebarSection, relativePath: string) => {
  ...
  const handleSaveEditedProfile = useCallback(
    async (formattedJson: string) => {
      ...
      await replaceApiProfileFile(editSession.relativePath, formattedJson, slicer);

      const compareArg =
        editSession.kind === 'filament' && isCustomFilamentProfile && compareFilamentPath
          ? compareFilamentPath
          : null;
      const refreshed = (await fetchApiResolve(editSession.relativePath, slicer, compareArg)).chain;
      setChain(refreshed);
      setEditSession((current) =>
        current
          ? {
              ...
              inherited: buildMergedProfileData(refreshed, refreshed.length - 2),
            }
          : null,
      );
    },
      ...
      >
        <div className='flex flex-wrap items-start justify-between gap-3'>
          <div>
            ...
            <p className='text-muted-foreground max-w-2xl text-xs'>{t('header.subtitle')}</p>
          ...
          <div className='flex flex-wrap items-end gap-3'>
            ...
            <label className='text-muted-foreground flex flex-col items-start gap-1 text-xs whitespace-nowrap'>
              ...
              <input
                ...
                onChange={(e) => setActiveExtruderIndex(Number(e.target.value) || 0)}
      ...
      <div className='flex min-h-0 flex-1 flex-col overflow-hidden md:flex-row'>
        ...
        >
          <div className='space-y-3 px-2 py-3'>
            <div className='flex flex-wrap gap-2'>
              <Tooltip.Provider delay={400}>
                <Tooltip.Root>
                  ...
                  >
                    ...
                    {apiOk === false ? t('controls.retryApi') : t('controls.checkConnection')}
                  </Tooltip.Trigger>
                  <Tooltip.Portal>
              ...
              <Button
                ...
                size='sm'
                className='min-w-26 flex-1'
                onClick={() => handleRefreshProfileList()}
            ...
            >
              {(['bambu', 'orca'] as const).map((source) => (
                ...
                >
                  {source === 'bambu' ? t('controls.slicerBambu') : t('controls.slicerOrca')}
                </Button>
              ))}
            </div>
            ...
            <div className='space-y-1.5'>
              {slicer === 'bambu' ? (
                ...
                <p className='text-muted-foreground text-xs'>{t('controls.orcaDefaultAccount')}</p>
              )}
          ...
          <div className='min-h-0 flex-1 overflow-y-auto py-2'>
            {apiOk !== true ? (
              <p className='text-muted-foreground px-2 py-4 text-sm'>{t('sidebar.connectFirst')}</p>
            ) : scanning ? (
              ...
              <ul className='flex flex-col'>
                {grouped.map(([mapKey, items], index) => {
                  ...
                  return (
                    <li key={mapKey} className='border-border border-b last:border-b-0'>
                      {index === firstProcessGroupIndex && firstProcessGroupIndex > 0 ? (
                        <div
                      ...
                      >
                        <CollapsibleTrigger
                          ref={(node) => {
                            if (node) {
                              sectionTriggerRefs.current.set(sidebarSection, node);
                            } else {
                              sectionTriggerRefs.current.delete(sidebarSection);
                            }
                          }}
                          type='button'
                          onKeyDown={(event) => handleSectionKeyDown(event, sidebarSection)}
                          className={cn(
                            ...
                            {items.map((p) => (
                              <li key={p.relativePath}>
                                <button
                                  ref={(node) => {
                                    if (node) {
                                      profileButtonRefs.current.set(p.relativePath, node);
                                    } else {
                                      profileButtonRefs.current.delete(p.relativePath);
                                    }
                                  }}
                                  type='button'
                                  onClick={() => setSelectedPath(p.relativePath)}
                                  onKeyDown={(event) =>
                                    handleProfileKeyDown(event, sidebarSection, p.relativePath)
                                  }
                                  aria-current={
                                    selectedPath === p.relativePath ? 'true' : undefined
                                  }
                                  className={cn(
          ...
          <div className='mt-2 min-h-0 flex-1 overflow-auto pb-4 pt-0'>
            {resolving ? (
              ...
              <ProfileTreeGrid
                ...
                showOnlyChangedLeaf={
                  isProcessProfile || isFilamentProfile ? showOnlyChanged : false
                }
                compareAccordion={
