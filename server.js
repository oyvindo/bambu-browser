...
function slicerContext(slicer) {
  ...
  const root = path.resolve(process.env[config.envName] || defaultStudioRoot(config.appDirectory));
...
const FDM_PROCESS_COMMON_RELATIVE = 'system/BBL/process/fdm_process_common.json';
/** Same logical path as BambuStudio/system/BBL/filament/fdm_filament_common.json */
const FDM_FILAMENT_COMMON_RELATIVE = 'system/BBL/filament/fdm_filament_common.json';
...
function isRelevantSystemDirectory(relativeDir, kind) {
  const parts = normalizeRelativePath(relativeDir).toLowerCase().split('/').filter(Boolean);
  if (parts[0] !== 'system') return false;
  if (parts.includes(kind)) return true;
  return kind === 'filament' && (parts[1] === 'custom' || parts[1] === 'orcafilamentlibrary');
}
...
async function resolveInheritanceRecursive(rootAbs, userFilePath, kind, visited, slicer = 'bambu') {
  ...
  const parentPath = await resolveParentRelativePath(rootAbs, p, inherits, kind, slicer);
  if (!parentPath) {
    throw new Error(
  ...
  const ancestors = await resolveInheritanceRecursive(rootAbs, parentPath, kind, visited, slicer);
...
async function listJsonInDir(dirAbs) {
  ...
  for (const e of ents) {
    if (e.isFile() && e.name.toLowerCase().endsWith('.json')) names.push(e.name);
  }
  return names.sort();
}
...
async function listProfilesForAccount(rootAbs, layout, account) {
  const base =
    layout === 'users' ? path.join(rootAbs, 'users', account) : path.join(rootAbs, 'user', account);
  const out = [];
  for (const sub of ['process', 'filament']) {
    ...
    const prefix = layout === 'users' ? `users/${account}/${sub}` : `user/${account}/${sub}`;
    for (const fileName of files) {
      out.push({
        userId: account,
        kind,
    ...
    if (sub === 'filament') {
      ...
      const basePrefix =
        layout === 'users' ? `users/${account}/filament/base` : `user/${account}/filament/base`;
...
async function handleRequest(req, res) {
  ...
  if (route === '/api/profile-file') {
    ...
    let relativePath;
    try {
      relativePath = editableUserProfilePath(studioRoot, rawPath, context.displayName);
  ...
  if (route === '/api/system-filaments') {
    try {
      ...
      return sendJson(res, 500, {
        error: e instanceof Error ? e.message : 'Failed to list system filaments',
      });
    }
  }
  ...
  if (route === '/api/resolve') {
    ...
    try {
      ...
      if (compareNorm) {
        ...
        chain = [...compareChain, { relativePath: normalized, data: customData }];
      } else {
        chain = await resolveInheritanceRecursive(studioRoot, normalized, kind, new Set(), slicer);
      }
...
if (require.main === module) {
  ...
  server.listen(PORT, '127.0.0.1', () => {
    console.log(`Bambu Studio / OrcaSlicer API listening on http://127.0.0.1:${PORT}`);
