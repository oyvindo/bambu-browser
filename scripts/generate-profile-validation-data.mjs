...
const studioRoot =
  process.env.BAMBUSTUDIO_ROOT ?? path.join(process.env.HOME ?? '', 'Documents', 'BambuStudio');
const outputPath = path.resolve('lib/bambu/profile-value-validation.generated.ts');
  ...
  for (const file of await jsonFilesUnder(directory)) {
    ...
    for (const [key, value] of Object.entries(parsed)) {
      ...
      for (const leaf of leaves(value)) {
        ...
        if (leaf === null || ['string', 'number', 'boolean'].includes(typeof leaf)) {
          const canonical = JSON.stringify(leaf);
          keyStats.values.set(canonical, (keyStats.values.get(canonical) ?? 0) + 1);
        }
      }
  ...
  for (const [key, keyStats] of kindStats.keys) {
    ...
    const boolish =
      ...
      rawValues.every((value) =>
        ['"0"', '"1"', '"true"', '"false"', 'true', 'false', '0', '1'].includes(value),
      );
    ...
    const categoryCandidates = rawValues
      ...
      .filter(
        (value) => typeof value === 'string' && numericValue(value) === null && value.length <= 80,
      );
    const uniqueCategories = [...new Set(categoryCandidates)].sort((a, b) => a.localeCompare(b));
    if (
    ...
    valueShapes.push([
      key,
      keyStats.array && keyStats.scalar ? 'mixed' : keyStats.array ? 'array' : 'scalar',
    ]);
  }

  return {
    fileCount: kindStats.fileCount,
    knownKeys: [...kindStats.keys.keys()].sort((a, b) => a.localeCompare(b)),
    numericBounds: stableObject(numericBounds),
    booleanKeys: booleanKeys.sort((a, b) => a.localeCompare(b)),
